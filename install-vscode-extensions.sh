...
install_extensions() {
    ...
    if command -v $program &> /dev/null; then
        # Print start message
        echo "$(date): Starting $program_name extension installation process"

        # Read extensions from the file and install them
        while IFS= read -r extension; do
            # echo "$(date): Installing extension $extension"
            $program_name --install-extension "$extension"
            if [ $? -eq 0 ]; then
        ...
        echo "$(date): $program_name extension installation process completed"
    else
